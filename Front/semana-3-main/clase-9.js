...
window.addEventListener('load', function () {
    ...
    const consultaUsuario = this.setTimeout(function(){
       const consulta = confirm ("Desea conocer mas musica?")
       if (consulta === true){
            window.open("https://open.spotify.com/");
       } else {
        const perfil = document.querySelector(".perfil")
        const parrafo = document.createElement("p");
        const contenido = document.createTextNode("Usuario oficial de Spotifront")
        parrafo.appendChild(contenido);
        perfil.appendChild(parrafo);
        parrafo.style.color = "green";
        
        parrafo.addEventListener("click", function(){alert("Gracias por confiar en nosotros")})
       }
    },2000);
